...
(def~asm
  ...
  (def~plc (register "/register")
    (def~lin ("Регистрация" :all supplier :clear)
      ...
      (def~fld (person    :update :all))
      (def~fld (phone     :update :all))
      (def~btn  ("Зарегистрироваться" :all :width 120)
        (with-obj-create (*USER* 'SUPPLIER (login password email name inn ogrn juridical-address actual-address person phone))
...
;;;;;
    (multi-grid ((:self :all)
                 ('(not :self)))
                ("Акции" :self sale (cons-inner-objs *SALE* (a-sales (gethash (cur-page-id) *USER*))))
...
;;;;;
  ...
  (def~plc (tender "/tender/:id")
    ...
    ;; resources
;;;;
    (multi-grid ((:owner :all)
                 ('(not :owner)))
                ("Ресурсы тендера" :owner tender-resource (cons-inner-objs *TENDER-RESOURCE* (a-resources (gethash (cur-page-id) *TENDER*))))
                   (def~fld (resource :width 400 :xref "tender-resource"))
